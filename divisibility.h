// VAMOS ESCREVER UMAS ROTINAS EM C PARA TESTAR A DIVISIBILIDADE DE UM INTEIRO
// POR NÚMEROS PRIMOS MEENORES QUE 30 ESTE ARQUIVO CONTÉM TAMBÉM ROTINAS ÚTEIS
// NA MANIPULAÇÃO DE INTEIROS EM C USANDO AS ROTINAS BÁSICAS DISPONIBILIZADAS
// AQUI É FÁCIL ESCREVER RÓTINAS PARA TESTAR A DIVISIBILIDADE POR OUTROS
// PRIMOS. UMA LISTA MAIS COMPLETA ESTÁ DISPONÍVEL
// EM:https://en.wikipedia.org/wiki/Divisibility_rule
...
int
digits_counter (uint64_t number)
{
...
uint64_t
positional_value (uint64_t number, int position)
{
...
uint64_t
extract_digits_interval (uint64_t number, int start_digit, int end_digit)
{
  // Variáveis locais
  int digit_number = digits_counter (number);
...
uint64_t
extract_last_digits (uint64_t number, int last_digits)
{
  // Variáveis locais
  int digit_number = digits_counter (number);
  // Procedimentos
  ...
  return extract_digits_interval (number, digit_number - last_digits + 1,
                                  digit_number);
};

// 3. Extraindo os primeiros dígitos de um número
uint64_t
extract_first_digits (uint64_t number, int first_digits)
{
  // Variáveis locais
  int digit_number = digits_counter (number);
  // Procedimentos
...
// 2
bool
divisible_by_2 (uint64_t number)
{
  // Variáveis locais
...
// 3
bool
divisible_by_3 (uint64_t number)
{
  // Váriáveis locais
  int sum_3 = 0;
  int digits_number = digits_counter (number);
...
// 5
bool
divisible_by_5 (uint64_t number)
{
  // Variáveis locais
  int last = extract_last_digits (number, 1);
  int digits_number = digits_counter (number);
...
// 7
bool
divisible_by_7 (uint64_t number)
{
  // Variáveis locais
  int digits_number = digits_counter (number);
...
// 11
bool
divisible_by_11 (uint64_t number)
{
  // Variáveis locais
  int digits_number = digits_counter (number);
  uint64_t sum_11 = 0;
...
// 13
bool
divisible_by_13 (uint64_t number)
{
  // Variáveis locais
  int digits_number = digits_counter (number);
  uint64_t sum_13;
...
// 17
bool
divisible_by_17 (uint64_t number)
{
  // Variáveis locais
  int digits_number = digits_counter (number);
  uint64_t sum_17;
...
// 19
bool
divisible_by_19 (uint64_t number)
{
  // Variáveis locais
  int digits_number = digits_counter (number);
  uint64_t sum_19;
...
// 23
bool
divisible_by_23 (uint64_t number)
{
  // Variáveis locais
  int digits_number = digits_counter (number);
  uint64_t sum_23;
...
// 29
bool
divisible_by_29 (uint64_t number)
{
  // Variáveis locais
  int digits_number = digits_counter (number);
  uint64_t sum_29;
