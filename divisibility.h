...
// AQUI É FÁCIL ESCREVER RÓTINAS PARA TESTAR A DIVISIBILIDADE POR OUTROS PRIMOS.
// UMA LISTA MAIS COMPLETA ESTÁ DISPONÍVEL
...
int digits_counter(uint64_t number) { return (int)log10(number) + 1; };
...
uint64_t extract_digits_interval(uint64_t number, int start_digit,
                                 int end_digit) {
  // Variáveis locais
  int digit_number = digits_counter(number);
...
bool divisible_by_3(uint64_t number) {
  ...
  if (sum_3 > 100) return divisible_by_3(sum_3);
...
bool divisible_by_7(uint64_t number) {
  ...
  if (sum_7 > 100) return divisible_by_7(sum_7);
...
bool divisible_by_11(uint64_t number) {
  ...
  // Teste
  for (i = 1; i <= digits_number; ++i) {
    if (i % 2 == 1) sum_11 += extract_digits_interval(number, i, i);
    if (i % 2 == 0) sum_11 -= extract_digits_interval(number, i, i);
  };
...
bool divisible_by_13(uint64_t number) {
  ...
  // Teste
  sum_13 = rest + 4 * last;
  if (sum_13 > 100) return divisible_by_13(sum_13);
...
bool divisible_by_17(uint64_t number) {
  ...
  if (sum_17 > 1000) return divisible_by_17(sum_17);
...
bool divisible_by_19(uint64_t number) {
  ...
  // Teste
  sum_19 = rest + 2 * last;
  if (sum_19 > 1000) return divisible_by_19(sum_19);
...
bool divisible_by_23(uint64_t number) {
  ...
  // Teste
  sum_23 = rest + 7 * last;
  if (sum_23 > 100) return divisible_by_23(sum_23);
...
bool divisible_by_29(uint64_t number) {
  ...
  if (sum_29 > 1000) return divisible_by_29(sum_29);
