//VAMOS CRIAR UM PROGRAMA QUE IMPLEMENTA O ALGORITMO BABY-STEP-GIANT-STEP USANDO BUSCA LINEARES
...
//********************************************************************************************************************************
//CABEÇALHO
#ifndef BABY_STEP_GIANT_STEP_LINEAR_H
#define BABY_STEP_GIANT_STEP_LINEAR_H
...
int64_t baby_step_giant_step_linear(int64_t, int64_t, int64_t);
...
int64_t baby_step_giant_step_linear(int64_t a, int64_t c, int64_t m){
  ...
  //Procedimentos
    ...
    for(j=0; j<n; ++j){
      ...
      for(i=0; i<=(n/2); ++i){
        ...
        if(value2==power_in[i].value){
          result=power_in[i].index*n-j;
          goto end;
        }

        if(value2==power_in[n-i].value){
          result=power_in[n-i].index*n-j;
          goto end;
        }
