// VAMOS CRIAR UM PROGRAMA QUE COMPUTA O NÚMERO DE CLASSE DE UM CORPO QUADRÁTICO
// DE DISCRIMINANTE POSITIVO USANDO FORMAS QUADRÁTICAS REDUZIDAS

/*
PARA CORPOS ALGÉBRICOS Q(√d) COM d>0 E SUFICIENTEMENTE PEQUENO É POSSÍVEL
COMPUTAR O NÚMERO DE CLASSE REALIZANDO-SE A CONTAGEM DE FORMAS QUADRÁTICAS
REDUZIDAS DE DISCRIMINANTE POSITIVO. UMA FORMA QUADRÁTICA ax²+bxy+cy², CUJO
DISCRIMINANTE Δ=b²-4ac>0 É REDUZIDA SE |√Δ -2|a||<b <√Δ.

SEJA r(a, b) = r, O MENOR INTEIRO TAL QUE r= b (mod 2a) E PARA UMA FORMA
QUADRÁTICA f=(a, b, c), A TRANSFORMAÇÃO ρ(a, b, c)=(c, r(-b, c), [(r²(-b,
c)-Δ)/(4c)]). EXISTE UM TEOREMA QUE GARANTE QUE APÓS UM NÚMERO FINITO DE PASSOS
A TRANSFORMAÇÃO ρ SEMPRE PRODUZ UMA FORMA QUADRÁTICA REDUZIDA, ALÉM DISSO DE f
FOR REDUZIDA PARA QUALQUER n INTEIRO SUFICIENTEMENTE GRANDE (ρ^n)f TAMBÉM É
REDUZIDA.

O NÚMERO DE ORBITAS LEVANDO EM CONTA A TRANSFORMAÇÃO ρ DEFINE O NÚMERO DE CLASSE
REDUZIDO h+(Δ) (O GRUPO DE CLASSES DE IDEAIS REDUZIDO LEVA EM COMSIDERAÇÃO
IMERSÕES DE UM CORPO ALGÉBRICO NO CONJUNTO DOS NÚMEROS REAIS). IDENTIFICANDO AS
FORMAS (a, b, c) COM (-a, b, -c) É POSSÍVEL COMPUTAR O NÚMERO DE CLASSE h(D).
...
//****************************************************************************************************************************************************************
// FUNÇÕES
// Função que checa se uma forma quadrática de discriminante positivo está
// reduzida
...
void reduce_quadratic_form_positive_discriminant(int64_t quadratic_form[]) {
  ...
  if (positive_discriminat_checker(quadratic_form) == true) return;
  ...
  reduction:
    ...
    while (true) {
      ...
      if (positive_a > sqrt(discriminant)) {
        if (r > (-1) * positive_a && r <= positive_a) break;
      };
  ...
  // Procedimento
    ...
    for (b = (d % 2); b < root; b += 2) {
      for (a = ((-1) * root) + 1; a < root; a++) {
        ...
        if (b > tester && (llabs((b * b) - d) % llabs(4 * a)) == 0) h++;
      };
    };
