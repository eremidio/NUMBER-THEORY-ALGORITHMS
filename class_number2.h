...
/*
O CÁLCULO DO NÚMERO DE CLASSE PARA UM CORPO ALGÉBRICO QUALQUER EM SUA FORMULAÇÃO GERAL É EXTREMAMENTE COMPLICADA.
ENVOLVENDO COMPLEXAS RELAÇÕES FUNCIONAIS ENVOLVENDO A FUNÇÃO ZETA DE DEDEKIND. 

PARA CORPOS QUADRÁTICOS IMAGINÁRIOS Q(√-n) COM n RAZOAVELMENTE PEQUENO (n<10^9) UMA POSSISIBILIDADE É
CONTABILIZAR O NÚMERO DE FORMAS QUADRÁTICAS REDUTÍVEIS COM DISCRIMINANTE -n, PARA ax²+bxy+cy², O
DISCRIMINANTE É DADO POR: Δ=b²-4ac.

UMA FORMA QUADRÁTICA É DITA SER REDUTÍVEL SE A SEGUINTE RELAÇÃO FOR SATISFEITA: |b|≲ a ≲ c.

PARA MAIORES INFORMAÇÕES: https://github.com/eremidio/NUMBER-THEORY-ALGORITHMS/blob/main/class_number.h
...
int64_t class_number_negative_discriminant2(int64_t d){
  ...
  //Restrição
  if(is_fundamental_discriminant(d)==false) return (-1);
...
w(d) é a função automorphism_number definida no noheader class_number.h que calcula o número de automorfismos que deixam uma forma
quadrática invariante. Esta fórmula fornce um resultado preciso.
*/

int64_t class_number_negative_discriminant3(int64_t d){

  //Restrição
  if(is_fundamental_discriminant(d)==true)  return (-1);
