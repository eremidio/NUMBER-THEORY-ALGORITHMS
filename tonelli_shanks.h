//VAMOS CRIAR UM PROGRAMA QUE IMPLEMENTA O ALGORITMO DE DE TONELLI SHANKS PARA ENCONTRAR RAÍZES QUADRADAS A MENOS DE UMA CONGRUÊNCIA
...
CONGRUÊNCIA E É BESEADO NO CRITÉRIO DE EULER PARA A EXISTÊNCIA DE QUADRADOS NÃO RESIDUAIS.
...
CASOS PARTICULARES p=3,5,7 (mod 8) (EQUIVALENTEMENTE p=3 (mod 4)) SÃO MAIS SIMPLES QUANDO E PODEM SER COMPUTADOS COM MENOR
ESFORÇO.
...
//**************************************************************************************************************************
//CABEÇALHO
#if !defined TONELLI_SHANKS_H
#define TONELLI_SHANKS_H
#include<stdint.h>
#include<stdio.h>
#include<stdbool.h>
...
int64_t tonelli_shanks(int64_t, int64_t);
int64_t modular_sqrt1(int64_t, int64_t);
int64_t modular_sqrt2(int64_t, int64_t);
int64_t modular_sqrt3(int64_t, int64_t);
...
int64_t modular_sqrt1(int64_t n, int64_t p){
 ...
 //Procedimentos
  ...
  //Encontrando um inteiro no intervalo 1, .., p-1 que é não quadrado perfeito modulo p
...
int64_t modular_sqrt2(int64_t n, int64_t p){
 ...
 //Variáveis locais
 int64_t r=mod_bin_pow(n, ((p+1)/4), p);
...
//Função que calcula uma raíz quadrada modular de um número usando o algoritmo de Tonelli-Shanks para o caso p=5 (mod 8)
int64_t modular_sqrt3(int64_t n, int64_t p){

 //Variáveis locais
 int64_t x=mod_bin_pow(n, ((p+3)/8), p);
 int64_t c=mod_bin_pow(x, 2, p);
 int64_t result;

 //Resultado
 if(n!=(c%p)){

  __int128_t temp=(x*mod_bin_pow(2, ((p-1)/4), p))%p;
  result=temp;
  return result;
...
int64_t tonelli_shanks(int64_t n, int64_t p){

 //Teste se n é uma classe residual quadrático módulo p usando o critério de Euler
 if(mod_bin_pow(n, ((p-1)/2), p)!=1)
  return 0;

 //Cálculo da raíz quadrada modular
 if((p%8)==5)
  return modular_sqrt3(n, p);
 if((p%4)==3)
