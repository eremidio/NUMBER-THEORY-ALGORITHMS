// VAMOS CRIAR UM PROGRAMA QUE IMPLEMENTA O ALGORITMO DE CURVAS ELIPTICAS PARA
// FATORAR NÚMEROS INTEIROS

/*
O MÉTODO DE FATORAÇÃO USANDO CURVAS ELÍPTICAS USA O FATO QUE EM CERTOS TIPOS DE
CURVAS UMA OPERAÇÃO GEOMÉTRICA DE ADIÇÃO DE PONTOS E MULTIPLICAÇÃO DE UM PONTO
POR UM ESCALAR DEFINE UMA ESTRUTURA QUE É UM GRUPO (ESTRUTURA ALGÉBRICA). A
IDEIA FUNDAMENTAL É CONSIDERAR CURVAS NA FORMA (NORMAL REDUZIDA) DE WEIERTRASS
y²=x³+ax+b, COM a E b REAIS; SOBRE UM CORPO FINITO DO PRIMEIRO TIPO (INTEIROS
COM AS OPERAÇÕES USUAIS USANDO ARITMÉTICA MODULAR).

NESTE CONTEXTO A CURVA EM QUESTÃO PASSA A TER UM NÚMERO FINITO DE ELEMENTOS,
INCLUINDO UM PONTO NO INFINITO, DENOTADO POR o.

NESTA SITUAÇÃO É POSSÍVEL OBTER UMA SITUAÇÃO NA QUAL A MULTIPLICAÇÃO DE PONTOS
POR UM ESCALAR EVENTUALMENTE IRÁ ATINGIR O PONTO NO INFINITO o. NESTE CÁLCULO
OPERAÇÕES ENVOLVENDO mdc DE DOIS INTEIROS SÃO PERFORMADOS E QUANDO PONTO o FOR
ATINGINDPO O CÁLCULO DO mdc EVENTUALMENTE IRÁ FORNECER UM FATOR NÃO TRIVIAL DO
NÚMERO A SER FATORADO.

ESTE ALGORITMO É UMA VARIANTE DO MÉTODO DE POLLARD RHO (p-1).

PARA MAIORES INFORMAÇÕES:
https://medium.com/@gaelfoppolo/elliptic-curve-cryptography-ecc-6af6142b0dd1
...
// Função que calcula o inverso modular de um número inteiro a (mod n) usando o
// algoritmo extendido de euclides
int64_t
modular_inverse (int64_t a, int64_t n)
{
  // Variáveis locais
...
int64_t
generate_random_number (int64_t number)
{
  // Variáveis locais
  int64_t result;
...
public:
  ...
  int64_t xp, yp, xq, yq, xr, yr,
      slope;    // Coordenadas de pontos na curva elíptica usados no cálculo
  int64_t a, b; // Parâmetros que definem a curva usada
  int64_t B; // Pârametro que define a profundidade do algoritmo na busca de
             // pontos na curva elíptica
  int64_t selection; // Número inteiro não congruente modulo o número a ser
                     // fatorado que serve de detecção de fatores não triviais
  int64_t factor1, factor2; // Fatores calculados usando este algoritmo
  int64_t number_trial; // Função que controla o número de testagens até o
                        // algoritmo encontrar um fator não trivial
  ...
  void
  set_new_curve (); // Função que reajusta a equação da curva para novos testes
  void pointwise_addition (); // Função que executa a adição de pontos em uma
                              // curva elíptica
  void point_scalar_multiplication (
      int64_t); // Função que executa a multiplição de pontos por um escalar em
                // uma curva elíptica
};
...
mainloop:
  ...
  if (number_trial > (MAX_TRIALS + 1))
    {
      std::cout
          << "Teste não encontrou fatores primos do número em questão!\n";
      return;
    };
  ...
  // Reinicializando o loop com uma nova curva caso fatores não tenham sio
  // encontrados
  if (factor1 == 0)
    {
      set_new_curve ();
...
void
elliptic_curve_method::setup ()
{
  std::cout << "Digite o número a ser fatorado: ";
  std::cin >> number;

  std::cout
      << "Digite o valor inicial da coordenada x0 de um ponto na curva: ";
  std::cin >> x0;
  std::cout
      << "Digite o valor inicial da coordenada y0 de um ponto na curva: ";
  std::cin >> y0;
  std::cout << "Digite o valor inicial do parâmetro a que define a curva a "
               "ser testada: ";
  std::cin >> a;
  std::cout << "Digite o uma fator de 2 a 250 que determina a profundidade da "
               "busca a ser realizada por fatores primos: ";
  std::cin >> B;
...
void
elliptic_curve_method::set_new_curve ()
{
...
void
elliptic_curve_method::pointwise_addition ()
{
  ...
  if (xp == xq && yp == yq)
    {
      ...
      // Calculo da inclinação da curva, uma checagem é executada para
      // verificar se um ponto não invertível foi encontrado
...
// Função que executa a multiplição de pontos por um escalar em uma curva
// elíptica
void
elliptic_curve_method::point_scalar_multiplication (int64_t n)
{
  // Variáveis locais
  int64_t i = 1;
