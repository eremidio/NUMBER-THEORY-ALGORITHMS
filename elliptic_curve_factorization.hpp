...
class elliptic_curve_method{
  ...
  int512_t discriminant;//Discriminante da curva elíptica
...
void elliptic_curve_method::run(){
  ...
  for(int64_t k=2; k<B; ++k){
    ...
    if(selection>1 && selection<number) break;
...
void elliptic_curve_method::point_scalar_multiplication(int64_t n){
  ...
  if(n==2){
    //Ajuste de variáveis
    pointwise_addition();
    if(selection>1 && selection<number) return;
  };
