...
LOGARITMOS DISCRETOS PODE SER CMPUTADOS USANDO-SE MULTIPLICAÇÃO ATÉ SE OBTER A RELAÇÃO DE CONGRUÊNCIA DESEJA. O ALGORITMO
DENOMINADO BABY_STEP_GIANT_STEP USA A SEGUINTE ABORDAGEM ESCREVEMOS k (b^k=c (mod m)) COMO k=in-j, COM i,j=0, ..., m^(1/2).
NESTA SITUAÇÃO TEMOS b^(in-j)=c (mod m) DONDE b^(in)= c(b^j) (mod m), PRE COMPUTANDO VALORES DE b^(in) E b^j, PODE-SE CHECAR
