...
A APLICAÇÃO MAIS IMPORTANTE DA FUNÇÃO DE MÖBIUS É A CHAMADA FÓRMULA DE
INVERSÃO: SEJAM f E g DUAS FUNÇÕES ARITMÉTICA TAIS QUE g(n)= Σf(d), ONDE n É UM
INTEIRO QUALQUER E d REPRESENTA O CONJUNTO DOS DIVISORES DE n ENTÃO VALE A
RELAÇÃO: f(n)= Σμ(d)g(n/d)
...
int
mobius_small_integer (uint64_t n)
{
  // Caso trivial
  if (n == 1)
    return 1;
...
int
mobius_large_integer (uint64_t n)
{
  // Variáveis locais
  int result = 1;
