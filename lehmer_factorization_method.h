...
/*
O MÉTODO DE LEHMER É UM EFICIENTE MÉTODO PARA FATORAR NÚMEROS SEMIPRIMOS (n=pq, COM p, q PRIMOS) ATÉ ~ 10^25 , ~ 10^30.

ESTE ALGORITMO USA FORMAS QUADRÁTICAS DO TIPO λN = x²-Dy², COM λ = -1, 1, 2 E D = -1, 1, 2, -2, 3, -3, 6, -6; PARA COMPUTAR NÚMEROS PRIMOS. SE DUAS SOLUÇÕES INTEIRAS
(x1, y1) (x2, y2) FOREM ENCONTRADAS PARA UMA DAS FORMAS QUADRÁTICAS EM QUESTÃO ENTÃO UM POSSÍVEL FATOR PRIMO É OBTIDO CALCULANDO-SE mdc(n, |x1y2-x2y1|).

O MÉTODO CONSISTE EM TESTAR MÚLTIPLOS VALORES DE y DENTRO DE LIMITES BEM ESTABELECIDOS USANDO-SE CRITÉRIOS BASEADOS NOS PARÂMETROS λ, D E TESTANDO SE x É QUADRADO PERFEITO. A ESCOLHA DA FORMA QUADRADA ADEQUADA É FEITAS BASEADO NA RELAÇÃO DE CONGRUÊNCIA r = n (mod 24).

PARA REFERÊNCIAS: https://www.ams.org/journals/mcom/1974-28-126/S0025-5718-1974-0342458-5/S0025-5718-1974-0342458-5.pdf
...
bool find_quadratic_representation(__int128_t, __int128_t, __int128_t, signed, signed, __int128_t[], __int128_t[]);
...
void select_quadratic_form_coefficients(__int128_t number, signed lambda_array[], signed D_array[]){
...
lambda_array[0]=1; lambda_array[1]=1; lambda_array[2]=1;
D_array[0]=(-2); D_array[1]=(-3); D_array[2]=6;

             };

if(residue==5){

lambda_array[0]=1; lambda_array[1]=2; lambda_array[2]=(-1);
D_array[0]=(-1); D_array[1]=(-6); D_array[2]=6;
...
lambda_array[0]=1; lambda_array[1]=(-1); lambda_array[2]=2;
D_array[0]=(-2); D_array[1]=3; D_array[2]=(-6);
...
lambda_array[0]=1; lambda_array[1]=1; lambda_array[2]=1;
D_array[0]=(-1); D_array[1]=(-3); D_array[2]=3;
...
lambda_array[0]=1; lambda_array[1]=1; lambda_array[2]=1;
D_array[0]=(-1); D_array[1]=(-2); D_array[2]=2;
...
void set_y_bounds(double* upper, double* lower, __int128_t number, signed lambda, signed D){
...
//Função que calcula uma representação de um inteiro a ser fatorado como uma forma quadrática  λN = x²-Dy²
bool find_quadratic_representation(__int128_t number, __int128_t upper, __int128_t lower, signed lambda, signed D,  __int128_t x_array[], __int128_t y_array[]){
...
//Procedimentos 
//1: Cálculo dos parâmetros λ, D das formas quadráticas λN = x²-Dy² a serem usadas no algoritmo
select_quadratic_form_coefficients(number, lambda_array, D_array);

//2: Loop principal: calculando rprtesentações de N usando formas quadráticas λN = x²-Dy²
...
factor_test=find_quadratic_representation(number, (__int128_t)upper_y, (__int128_t)lower_y, lambda, D, x_array, y_array);
