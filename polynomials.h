//VAMOS CRIAR UM PROGRAMA QUE IMPLEMENTA UMA CLASSE DE POLINÔMIOS DE UMA ÚNICA VARIÁVEL EM C++ (ÚTIL IMPLEMENTAR ALGORITMOS QUE USAM POLINÔMIOS)

/*
ESTA CLASSE SERÁ PARA USO DE POLINÔMIOS QUE ASSUMEM VALORES NO CORPO DOS NÚMEROS REAIS.
...
void set_coefficients();//Função que define os coeficientes de um polinômios
...
for(j=0; j<polynomial_powers[i]; ++j)
parcel*=value;
...
if(difference<=0){
result.polynomial_coefficients.push_back(polynomial_coefficients[i]+p2.polynomial_coefficients[i-temp]);
++i;
                };
...
if(difference<=0){
result.polynomial_coefficients.push_back(p2.polynomial_coefficients[i]+polynomial_coefficients[i-temp]);
++i;
                };
...
if(result.degree==degree){
for(auto x:polynomial_powers)
result.polynomial_powers.push_back(x);
                         };
if(result.degree==p2.degree){
for(auto y:p2.polynomial_powers)
result.polynomial_powers.push_back(y);
                            };
...
if(difference>0){
difference--;
result.polynomial_coefficients.push_back((-1)*p2.polynomial_coefficients[i]);
++i;
                };

if(difference<=0){
result.polynomial_coefficients.push_back((-1)*p2.polynomial_coefficients[i]+polynomial_coefficients[i-temp]);
++i;
                };
...
if((i+j)==k)
calculated_coefficient=calculated_coefficient+(polynomial_coefficients[i]*p2.polynomial_coefficients[j]);
                                                  };
                                               };
...
result.polynomial_coefficients.push_back(polynomial_coefficients[(polynomial_coefficients.size()-1)]*p2.polynomial_coefficients[(p2.polynomial_coefficients.size()-1)]);
