...
bool square_free_odd_checker(uint64_t number){
//Variável local
...
//Função que calcula soluções inteiras para a forma quadrática 4x²+y²=n, com n e y ímpares
uint64_t theorem1(uint64_t number){
//Variáveis locais
...
//Função que calcula soluções inteiras para a forma quadrática 3x²+y²=n, com n e x ímpares; e y par
uint64_t theorem2(uint64_t number){
//Variáveis locais
uint64_t x, y, limit;//Variáveis de iteração
uint64_t temp, result;
...
//Função que calcula soluções inteiras para a forma quadrática 3x²-y²=n, com x e y alternadamente par/ímpar ou vice-versa
uint64_t theorem3(uint64_t number){
//Variáveis locais
uint64_t x, y, limit;//Variáveis de iteração
uint64_t temp, result;
...
void atkin_algorithm(uint64_t atkin_int){

//Variáveis locais
bool atkin_array[atkin_int];//Array de booleanos
bool atkin_test;//Variável que determina se um número é primo ou não
uint64_t atkin_residue, atkin_value;//Módulo 60 e número de soluções as formas quadrátricas dos teoremas
...
//Casos não triviais encontrados o algoritmo prossegue com o cálculo do resíduo módulo 60
atkin_residue=i%60;

//Caso 1: teorema 1; n≡ 1, 13, 17, 29, 37, 41, 49, 53 mod(60)
if(atkin_residue==1 || atkin_residue==13 || atkin_residue==17 || atkin_residue==23 || atkin_residue==37 || atkin_residue==41 || atkin_residue==49 || atkin_residue==53){
...
if(atkin_residue==11 || atkin_residue==23 || atkin_residue==47 || atkin_residue==59){
