//VAMOS CRIAR UMA CLASSE DE MATRIZES PARA OPERAÇÕES BÁSICAS COMO COMO SOMA, SUBTRAÇÃO, MULTIPLICAÇÃO E TRANSPOSIÇÃO.
//CLASSE ÚTIL PARA PROBLEMAS DE ÁLGEBRA LINEAR BÁSICA E PARA ALGORITMOS COMPLEXOS QUE REQUER MANIPULAÇÃO BÁSICA DE MATRIZES

/*
OPERAÇÕES MAIS COMPLEXAS COMO CÁLCULO DE MATRIZES INVERSAS E CÁLCULO DO DETERMINANTE PODEM SER DEFINIDOS SOBRE ELEMENTOS DESTA CLASSE USANDO ALGORITMOS COMO A ELIMINAÇÃO
DE GAUSS-JORDAN, ENTRE OUTROS.

USAMOS AQUI INDEXAGEM DE [0 ...  (linhas-1)]  [0 ...  (colunas-1)] PARA  REFERENCIAR ELEMENTOS DA MATRIZ. O PRIMEIRO ELEMENTO (1,1) DA MATRIZ, POR EXEMPLO, É
REFERENCIADO PELOS INDEXES (0,0).
...
std::vector<std::vector<T>> matrix;//Vamos implementar matrizes como arrays dinâmicos (vetores) de arrays dinâmicos

//Construtores e destruídores
Matrix(int64_t, enum matrix_types);//Matríz nula e matriz identidade
Matrix(int64_t, int64_t);//Matriz especificados o número de linhas e colunas
...
void reshape(int64_t, int64_t);//Função que redimensiona uma matriz adicionado linhas abaixo e colunas a direita da matriz
...
Matrix<T> slice(int64_t, int64_t, int64_t, int64_t);//Função que retorna uma submatriz da matriz
Matrix<T> comatrix(int64_t, int64_t);//Função que gera uma matriz quadrada removendo a linha e a coluna que um elemento se encontra
T get_element(int64_t, int64_t);//Função que retorna um elemento da matriz

//Operações de entrada e saída de elementos
void set_element(int64_t, int64_t, T);//Função que define o valor de um elemento na matriz
...
Matrix<T> scalar_multiplication(double);//Multiplicação de matrizes por escalares
...
//Função que redimensiona uma matriz adicionado linhas abaixo e colunas a direita da matriz
template<typename T>
...
//Procedimentos
for(i=0; i<columns; i++)
row_matrix.set_element(0,i, matrix[k][i]);
...
//Procedimentos
for(i=0; i<rows; i++)
column_matrix.set_element(i,0, matrix[i][k]);
...
template<typename T>
Matrix<T> Matrix<T>::slice(int64_t start_row, int64_t end_row, int64_t start_column, int64_t end_column){
//Variáveis locais
int64_t i,j;
...
//Função que gera uma matriz quadrada removendo a linha e a coluna que um elemento se encontra
template<typename T>
...
for(j=0; j<new_columns; ++j){
if(i<k1 && j<k2)
reduced_matrix.set_element(i,j, matrix[i][j]);
if(i>=k1 && j<k2)
reduced_matrix.set_element(i,j, matrix[i+1][j]);
if(i<k1 && j>=k2)
reduced_matrix.set_element(i,j, matrix[i][j+1]);
if(i>=k1 && j>=k2)
...
for(i=0; i<rows; ++i){
for(j=0; j<columns; ++j){
std::cout<<"Digite o valor do elemento ("<<i<<","<<j<<") da matriz: ";
std::cin>>element;
...
//Somando o produto da i-ésima linha de uma matriz pela j-ésima coluna da outra matriz
for(k=0; k<m2.rows; ++k)
...
//Inteiros
template<typename T>
Matrix<T> Matrix<T>::scalar_multiplication(int64_t k){
//Variáveis locais
...
//Reais
template<typename T>
Matrix<T> Matrix<T>::scalar_multiplication(double k){
//Variáveis locais
Matrix<T> result(rows, columns);
int64_t i, j;
