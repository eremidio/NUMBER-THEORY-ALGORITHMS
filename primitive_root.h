// VAMOS CRIAR UM PROGRAMA QUE IMPLEMENTA ROTINAS PARA SE CALCULAR RAÍZES
// PRIMITIVAS DE UM NÚMERO INTEIRO n
...
POTÊNCIA DE m. EXEMPLO: SEJA n=5, COPRIMOS COM ELE SÃO {2, 3, 4}. TEMOS: 2^1 =
2 (mod 5) 2^2 = 4 (mod 5) 2^3 = 3 (mod 5) 2^4 = 1 (mod 5) 2^5 = 2 (mod 5) 2^6 =
4 (mod 5) 2^7 = 3 (mod 5) 2^8 = 1 (mod 5) 2^9 = 2 (mod 5) 2^10 = 4 (mod 5)
...
FOR: 1, 2, 4, p^k, OU 2p^k, COM p PRIMO E k INTEIRO. TAL OBSERVAÇÃO É UM
INDÍCIO DOS NÚMEROS QUE POSSUEM RAÍZES PRIMITIVAS.
...
1. PARA TESTAR UM CANDIDATO m A RAÍZ PRIMITIVA DE n PODEMOS USAR O FATO DE QUE
A ORDEM MULTIPLICATIVA DE m (MENOR EXPOENTE k TAL QUE m^k=1 (mod n)) É IGUAL A
φ(n)=λ(n) (λ(n) É FUNÇÃO TOTIENTE REDUZIDA OU FUNÇÃO LAMBDA DE CARMICHAEL).
2. PODEMOS TAMBÉM COMPUTAR m^(φ(n)/p) PARA TODOS OS FATORES PRIMOS p DE φ(n),
SE PARA TODO p O RESULTADO FOR DIFERENTE DE 1 UMA RAÍZ PRIMITIVA DE n FOI
ENCONTRADA.

O NÚMERO DE RAÍZES PRIMITIVAS DE UM NÚMERO n É φ(φ(n)). PARA UM PRIMO p SUA
SOMA É IGUAL A FUNÇÃO DE MÖBIUS μ(p-1) E SEU PRODUTO É IGUAL A 1 (mod p). SE m
FOR UMA RAÍZ DE PRIMITIVA DE p TAMBÉM O É DE p^k PARA k INTEIRO A MENOS QUE:
...
bool
primitive_root_checker (uint64_t n)
{
  // Casos triviais
  if (n == 1 || n == 2 || n == 4)
    return true;
  ...
  // Testando porfatores primos ímpares (um número não pode ser divisível por
  // dois ímpares diferentes)
...
primitive_root (uint64_t n)
{
  ...
  // Procedimentos iterando sobre possíveis candidatos (usaremos o algoritmo de
  // Euclides definido em euler_totient_function.h)
  for (i = 1; i < n; ++i)
    {
      ...
      if (j == validator)
        {
          root_number++;
          primitive_root_array = (uint64_t *)realloc (
              primitive_root_array, root_number * sizeof (uint64_t));
