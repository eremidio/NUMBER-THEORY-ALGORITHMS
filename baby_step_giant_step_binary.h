//VAMOS CRIAR UM PROGRAMA QUE IMPLEMENTA O ALGORITMO BABY-STEP-GIANT-STEP USANDO BUSCA BINÁRIAS
...
int64_t binary_search(int64_t, struct log_map[], int64_t);
...
//Função que implementa rotinas para buscas binárias em arrays de inteiros de 64-bits
int64_t binary_search(int64_t target, struct log_map array[], int64_t array_size) {

  //Variáveis locais
  int64_t lower = 0, upper = (array_size - 1), mid;
...
int64_t baby_step_giant_step_binary(int64_t a, int64_t c, int64_t m){
  ...
  //Procedimentos
    ...
    for(j=0; j<n; ++j){
      ...
      i=binary_search(value2, power_in, array_size);
      if(i>=0){
        result = ((power_in[i].index*n)-j);
        break;       
              };
