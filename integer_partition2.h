// VAMOS CRIAR UM PROGRAMA QUE GERA DIAGRAMAS DE YOUNG E AS RESPECTIVAS PARTIÇÕES DE UM NÚMERO INTEIRO

/*

PARA MAIORES INFORMAÇÕES: https://jeromekelleher.net/generating-integer-partitions.html
...
void print_partition(int array[], int size) {
  // Variáveis locais
  int i;
  // Procedimento
  for (i = 0; i < size; ++i) {
    if (i == 0 && array[0] > 0) printf("%d", array[0]);
    if (i > 0 && array[i] > 0) printf("+%d", array[i]);
  };
  printf("\n");
};

// Função que printa os diagramas de Young associado a um número inteiro
void print_young_diagram(int array[], int size) {
  // Variáveis locais
  int i, j, k;
  // Procedimento
  for (i = 0; i < size; ++i) {
...
void generate_partition(int m, enum show x) {
  ...
  while (true) {
    ...
    if (x == Yes) print_young_diagram(partition, k + 1);
