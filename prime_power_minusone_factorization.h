// VAMOS CRIAR UM PROGRAMA QUE FATORA NÚMEROS INTEIROS DA FORMA N=(b^n)-1, COM
// b, n INTEIROS

/*
PARA NÚMEROS DA FORMA N=(b^n)-1 O SEGUINTE TEOREMA É ÚTIL EM SUA DECOMPOSIÇÃO EM
FATORES PRIMOS: PARA b, n INTEIROS E N=(b^n)-1, OS FATORES PRIMOS DE p N
SATISFAZEM A SEGUINTE CONDIÇÃO: [1] p É UM DIVISOR DE N'=(b^d)-1,ONDE d É UM
DIVISOR NÃO TRIVIAL DE n, [2] p É DA FORMA p= 1 (mod n), OU SE p>2  E n FOR
ÍMPAR p= 1 (mod 2n).
...
//****************************************************************************************************************************************************
// FUNÇÕES
// Função que calcula fatores primos de um inteiro N=(b^n)-1, baseados na
// fatoração parcial de N'=(b^d)-1 com d|n
void proper_divisors_partial_factorization(uint64_t base, uint64_t power,
                                           uint64_t* N) {
  // Variáveis locais
  uint64_t N_ = (*N);
  ...
  for (i = 0; i < k; i++) {
    if ((N_ % factor_array[i]) == 0) {
      ...
      while ((N_ % factor_array[i]) == 0) N_ /= factor_array[i];
    };
  };
...
// Função que busca por fatores primos da forma p= 1 (mod n) ou p= 1 (mod 2n) se
// n for ímpar
void congruence_divisors_factorization(uint64_t power, uint64_t* N) {
  ...
  while (divisor < (root + 2 * step)) {
    ...
    if ((N_ % divisor) == 0) {
      printf("Fator de %lu encontrado: %lu\n", (*N), divisor);
      while ((N_ % divisor) == 0) N_ /= divisor;
    };
  };

  // Condição que determina a primalidade de N
  if (N_ < (*N) && N_ > 1) printf("Fator de %lu encontrado: %lu\n", (*N), N_);

  if (N_ == (*N)) printf("%lu é primo.\n", N_);
};
...
void prime_power_minusone_factorization(uint64_t base, uint64_t power) {
  ...
  exponent_divisors =
      (uint64_t*)realloc(exponent_divisors, sizeof(uint64_t) * k);
  ...
  if (N1 > 1) congruence_divisors_factorization(power, &N1);
