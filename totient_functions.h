...
A FUNÇÃO λ DE CARMICHAEL OU FUNÇÃO TOTIENTE REDUZIDA É DEFINIDA COMO O MENOR INTEIRO λ(n) TAL QUE TODOS OS NÚMEROS COPRIMOS a COM n NO INTERVALO 2,..., (n-1); SATISFAZEM
...
__int128_t mod_pow(uint64_t, uint64_t, uint64_t);
...
__int128_t mod_pow(uint64_t a, uint64_t b, uint64_t n){
//Variáveis locais
__int128_t result=1;
...
for(; i<n; ++i){
if(mod_pow(coprime_array[0], i, n)==1){
exponent=i;
break;
                                      };
               };
