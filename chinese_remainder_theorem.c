// VAMOS CRIAR UM PROGRAMA EM C QUE CALCULA O VALOR DE x NO TEOREMADE RESÍDUO
// CHINÊS COMPILAR ESTE PROGRAMA COM O COMANDO: gcc -o
...
PROVIDO QUE O TERMOS DA SEQUÊNCIA n(1), n(2), ... ; SEJAM RELATIVAMENTE PRIMOS
E PARA TODOi=1,2,...; 0<a(i)<n(i). EXISTE UM ÚNICO x QUE SATISFAZ ESTAS
RELAÇÕES A MENOS DE UMA RELAÇÃO DE CONGRUÊNCIA COM N = n(1).n(2).(...).n(k).

VÁRIAS TÉCNICAS EXISTEM PARA CALCULAR O VALOR DE x QUE SATIFAZ ESTAS RELAÇÕES.
O TEOREMA PODE SER EXTENDIDO PARA CASOS EM QUE n(1), n(2), ... ; NÃO SÃO
...
// FUNÇÕES
uint64_t
chinese_residue_theorem (uint64_t a[], uint64_t n[], uint64_t k)
{
  // Variáves locais
  uint64_t x, status;
  ...
  while (true)
    {
      ...
      if (status == 0) // Condição para que todas as relações de congruências
                       // sejam satisfeitas
        break;
      else
        x += n[0];
...
main ()
{
  ...
  for (i = 0; i < k; ++i)
    {
      printf ("Entre com o valor de n(%li) (o maior dos n(i)'s deve vir "
              "primeiro): ",
              i);
      scanf ("%lu", &n[i]);
