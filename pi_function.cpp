// VAMOS IMPLEMENTAR UM PROGRAMA QUE CALCULA A FUNÇÃO π(x) QUE CALCULA O NÚMERO
// DE PRIMOS ATÉ UM DADO LIMITE DE 10⁶ COMPILAR ESTE PROGRAMA COM O COMANDO:
// g++ -o pi_function pi_function.cpp O PRESENTE CÓDIGO PRODUZ PEQUENOS ERROS
// PERCENTUAIS DEVIDO AO POSSÍVEL MAL AJUSTE DE PARÂMETROS

/*

A FUNÇÃO π(x) É UMA DAS FUNÇÕES MAIS IMPORTANTES EM MATEMÁTICA E CONTABILIZA O
NÚMERO DE PRIMOS DADO UM CERTO LIMITE.

O CHAMADO TEOREMA DE NÚMEROS ṔRIMOS ESTABELECE QUE PARA VALORES GRANDES DE x
π(x)=x/ln(x) OU π(x)=li(x). ln(x) DENOTA O LOGARITMO NATURAL. li(x) DENOTA A
CHAMADA INTEGRAL LOGARÍTMICA, A INTEGRAL DEFINIDA DE 1/ln(x) DE 0 ATÉ x.

A FUNÇÃO π(x) É EXTREMAMENTE IMPORTANTE PARA ESTUDAR A DENSIDADE DE NÚMEROS
PRIMOS NO CONJUNTODOS INTEIROS POSITIVOS, ESTANDO CONECTADO DIRETAMENTE A
CHAMADA HIPÓTESE DE RIEMANN QUE ESTABELECE K]LIMITES PRECISOS PARA A FUNÇÃO
π(x).

DIVERSOS ALGORITMOS COMO A PENEIRA DE ERATÓSTENES PODEM SER USADOS PARA
CONTABILIZAR A FUNÇÃO π(x). NESTE PROGRAMA IMPLEMENTAMOS UM MÉTODO MISTO USANDO
A PENEIRA DE ERATÓSTENES E O ALGORITMO DE MEISSEL-LEHMER.
...
is_prime (uint64_t n)
{
  ...
  for (uint64_t i = 101; i < sqrt_n + 1; i += 30)
    {
      if ((n % i) == 0 || (n % (i + 2)) == 0 || (n % (i + 6)) == 0
          || (n % (i + 8)) == 0 || (n % (i + 10)) == 0 || (n % (i + 12)) == 0
          || (n % (i + 16)) == 0 || (n % (i + 18)) == 0 || (n % (i + 22)) == 0
          || (n % (i + 26)) == 0 || (n % (i + 28)) == 0)
        return false;
    };
  return true;
...
// Função de contagem de primos para números pequenos usando a peneira de
// Eratóstenes Função de contagem para pequenos números primos
unsigned long
sieve_counting (unsigned long n)
{
...
public:
  ...
  void fill_prime_buffer (); // Função que estoca os número primos até
                             // n^(2/3)em um vetor
...
pi::pi_function ()
{
  if (n <= SIEVE_MAX)
    ...
    {
      result
          = phi (n, lower) + (0.5 * (upper + lower - 2) * (upper - lower + 1));
...
// Função Φ(m, n)
unsigned long
pi::phi (unsigned long a, unsigned long b)
{
