...
(REPRESENTADO POR UMA TRIPLA f=(a, b, c)) DE DISCRIMINANTE Δ=b²-4ac, DEFINIMOS e=(1, 0, |Δ|/4) PARA -Δ PAR E e=(1, 1, |1-Δ|/4) PARA -Δ ÍMPAR COMO A UNIDADE RELATIVA A
OPERAÇÃO DE COMPOSIÇÃO DE FORMA QUADRÁTICAS. f=(a, b, c) É UMA FORMA AMBÍGUA SE f FOR EQUIVALENTE A SUA INVERSA g=(a, -b, c) A MENOS DE UMA TRANSFORMAÇÃO LINEAR. 
...
FORMAS AMBÍGUAS. AS FORMAS QUADRÁTICAS e LISTADAS ACIMA PRODUZEM FATORAÇÕES TRIVIAIS.
...
int64_t triple_euclides_algorithm(int64_t a, int64_t b, int64_t c){
  ...
  //Resultado
  if(two_gcd==1) return 1;
  else euclides_algorithm(two_gcd, c);
