...
void fill_prime_buffer(std::vector<uint64_t>& prime_vector, uint64_t upper_bound, uint64_t lower_bound){
  ...
  //Procedimentos
    ...
    //Preenchendo o buffer de primos
...
int64_t generate_random_number(){
  ...
  //Variáveis locais
  int64_t result;
  const int64_t limit=10000000000000000;
  ...
  //Procedimentos
    ...
    std::uniform_int_distribution<int64_t> elliptic_distribution(10000, limit);
...
void extract_bits(std::vector<short>& bit_vector, int512_t n){
  ...
  //Procedimentos
    ...
    //Extraindo os bits 
    while(n>0){
      if(n&1) bit_vector.push_back(1);
      else bit_vector.push_back(0);
      n>>=1;
    };
