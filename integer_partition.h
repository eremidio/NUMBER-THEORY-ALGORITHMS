...
COMO UMA SOMA DE NÚMEROS INTEIROS NÃO NEGATIVOS. POR EXEMPLO AS PARTIÇÕES DE
n=4 SÃO: 1+1+1+1, 2+2, 2+1+1, 3+1, 4.
...
DIAGRAMAS DE YOUNG SÃO PARTICULARMENTE ÚTEIS PARA REPRESENTAR FUNÇÕES
SIMÉTRICAS E SÃO MUITO IMPORTANTES EM TEORIA DE REPRESENTAÇÃO DE CONJUNTOS,
TENDO APLICAÇÕES IMPORTANTES EM FÍSICA TEÓRICA (MECÂNICA QUÂNTICA E TEORIA
QUÂNTICA DE CAMPOS).
...
VALOR PODE SER EXPLICITAMENTE CALCULADO USANDO RELAÇÕES RECURSIVAS OU
APROXIMADO PARA GRANDES VALORES USANDO EXPRESSÕES QUE CONVERGEM PARA O VALOR
CORRETO COM GRANDE PRECISÃO. UMA FUNÇÃO GERADORA USANDO SÉRIES GEOMÉTRICAS PODE
SER USADA PARA CALCULAR OS VALORES DE p(n) PARA UMA DADO VALOR DE n.
...
unsigned int sum_of_divisors (
    unsigned int); // FUNÇÃO QUE CALCULA A SOMA DE DIVISORES DE UM NÚMERO
unsigned int partition_function (
    unsigned int); // FUNÇÃO QUE CALCULA O NÚMERO DE PARTIÇÕES DE UM INTEIRO
...
unsigned int
partition_function (unsigned int n)
{
  // Casos triviais
  if (n == 0 || n == 1)
    return 1;
