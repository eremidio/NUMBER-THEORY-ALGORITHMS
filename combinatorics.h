...
uint64_t
repeated_permutation (uint64_t n, uint64_t repetition_number_array[],
                      uint64_t size)
{
  // Variáveis locais
  uint64_t result, i;
...
// Arranjo simples
uint64_t
arrangement (uint64_t n, uint64_t k)
{
  // Restrição
  assert (n >= k);
  // Procedimntos
...
// Combinação simples
uint64_t
combination (uint64_t n, uint64_t k)
{
  // Restrição
  assert (n >= k);
  // Procedimntos
