// VAMOS ESCREVER UM PROGRAMA EM C QUE É USADO PARA COMPUTAR A FUNÇÃO TOTIENTE
// DE EULER TAMBÉM CONHECIDA COMO FUNÇÃO φ DE EULER

/*
PARA UM ARGUMENTO n A FUNÇÃO φ(n) CONTABALIZA QUANTOS INTEIROS NO INTERVALO [1,
2, ..., n] SÃO RELATIVAMENTE PRIMOS COM n. ISTO É, DADO UM NÚMERO a NO
INTERVALO 1, 2,..., n, φ(n) CONTABILIZA QUANTOS NÚMEROS SATISFAZEM A CONDIÇÃO
mdc(a, n)=1.

A FUNÇÃO φ(n) É MULTIPLICATIVA: φ(n)φ(m)=φ(nm) SE n E m FOREM  RELATIVAMENTE
PRIMOS. PARA UM NÚMERO PRIMO p: φ(p)=p-1 (APENAS 1 É RELATIVAMENTE PRIMO COM
n).
...
ENVOLVENDO FATORES PRIMOS DE n. PARA MAIORES
...
is_prime (uint64_t n)
{
  ...
  if (n % 2 == 0 || n % 3 == 0 || n % 5 == 0 || n % 7 == 0 || n % 11 == 0
    ...
    return false;
  if (n % 43 == 0 || n % 47 == 0 || n % 53 == 0 || n % 59 == 0 || n % 61 == 0
      || n % 67 == 0 || n % 71 == 0 || n % 73 == 0 || n % 79 == 0
      || n % 83 == 0 || n % 89 == 0)
    return false;
  if (n % 97 == 0)
    return false;
...
uint64_t
totient_function_small_numbers (uint64_t n)
{
...
// multiplicativa da função φ para ≲250000²) Essa função não
uint64_t
totient_function_large_numbers (uint64_t n)
{
  // Variáveis locais
...
uint64_t
euler_totient_function (uint64_t n)
{
  if (n > PHI_MAX)
