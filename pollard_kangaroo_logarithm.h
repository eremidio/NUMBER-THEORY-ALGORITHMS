...
/*
O ALGORITMO CANGURU (λ) DE POLLARD PARA O CÁLCULO DE LOGARITMO DISCRETO USA DUAS SUBSEQUÊNCIAS {x(0), ..., x(N)} E {y(0), y(1), ...} PARA O CÁLCULO DO LOGARITMO
DISCRETO. DADO UM GRUPO CÍCLICO G DE ORDEM n, COM UM GERADOR g E UM ELEMENTO h O OBJETIVO É DETERMINAR UM x TAL QUE g^x=g*...*g=h. 
O ALGORITMO USA UMA FUNÇÃO QUE GERA NÚMEROS PSEUDOALETÓRIOS f:G->S, ONDE S É UM CONJUNTO PRÉ-DEFINIDO PARA DETERMINAR UMA RELAÇÃO DO TIPO g^a=h(g^b) QUE IMPLICA NA
RELAÇÃO h=g^(b-a), UMA DAS SEQUÊNCIAS É INICIALIZADA COM O VALOR y(0)=h. ESTE ALGORITMO É PARTICULARMENTE ÚTIL PARA SE DETERMINAR LOGARITMOS DISCRETOPS EM UM DADO INTERVALO [a, (a+1), ... , (b-1), b], INCLUINDO O INTEVALO {0, 1, ..., (n-1)} QUE ABRANGE OS POSSIVEIS VALORES.
...
//Variáveis locais
int64_t result;
int64_t x0=mod_bin_pow((uint64_t)g, ((uint64_t)(N-1)), (uint64_t)N);
...
//1ª iteração
x1=x0;
x2=((x1%N)*mod_bin_pow((uint64_t)g, ((uint64_t)pseudo_randomwalk(x1, g, h, N)), (uint64_t)N))%N;
...
xn=mod_bin_pow((uint64_t)g, ((uint64_t)(d+N-1)), (uint64_t)N);
y0=h;
ds=0;
y1=y0;
...
y2=((y1%N)*mod_bin_pow((uint64_t)g, ((uint64_t)pseudo_randomwalk(y1, g, h, N)), (uint64_t)N))%N;
