...
/*
O ALGORITMO CANGURU (λ) DE POLLARD PARA O CÁLCULO DE LOGARITMO DISCRETO USA DUAS
SUBSEQUÊNCIAS {x(0), ..., x(N)} E {y(0), y(1), ...} PARA O CÁLCULO DO LOGARITMO
DISCRETO. DADO UM GRUPO CÍCLICO G DE ORDEM n, COM UM GERADOR g E UM ELEMENTO h O
OBJETIVO É DETERMINAR UM x TAL QUE g^x=g*...*g=h.

O ALGORITMO USA UMA FUNÇÃO QUE GERA NÚMEROS PSEUDOALETÓRIOS f:G->S, ONDE S É UM
CONJUNTO PRÉ-DEFINIDO PARA DETERMINAR UMA RELAÇÃO DO TIPO g^a=h(g^b) QUE IMPLICA
NA RELAÇÃO h=g^(b-a), UMA DAS SEQUÊNCIAS É INICIALIZADA COM O VALOR y(0)=h. ESTE
ALGORITMO É PARTICULARMENTE ÚTIL PARA SE DETERMINAR LOGARITMOS DISCRETOS EM UM
DADO INTERVALO [a, (a+1), ... , (b-1), b], INCLUINDO O INTEVALO {0, 1, ...,
(n-1)} QUE ABRANGE OS POSSIVEIS VALORES.

PARA MAIORES INFORMAÇÕES:
https://link.springer.com/content/pdf/10.1007/s001450010010.pdf
...
int64_t pollard_kangaroo_logarithm(int64_t g, int64_t h, int64_t N) {
  ...
  while (true) {
    ...
    if (ds > (N - 1 + d)) {
      printf(
          "O algoritmo não encontrou o logaritmo de %li (mod %li) na base "
          "%li\n",
          h, g, N);
      return (-1);
    };
