// VAMOS CRIAR UM PROGRAMA QUE IMPLEMENTA O ALGORITMO DE BAILLIE-PSW PARA
// TESTAR A PRIMALIDADE DE UM NÚMERO INTEIRO
...
//********************************************************************************************************************
// FUNÇÕES
// Função que testa se um número é divisível por pequenos números primos,
// usaremos primos até 100
bool
small_prime_factor_test (uint64_t n)
{
...
// Função que realiza o teste de Lucas Lehmer para a primalidade dos chamados
// números de Merssene da forma 2^p-1
bool
lucas_lehmer_test (uint64_t n)
{
...
// Função que realiza o teste de Baillie para testar a primalidade de um número
// primo
bool
baillie_test (uint64_t n)
{
  ...
  // Checando se o número de questão pode ser escrito como a diferença de
  // quadrados, Variáveis locais
  uint64_t i;
...
bool
lucas_sequence_test (int64_t n, int64_t P, int64_t Q)
{
  ...
  while (k <= n)
    {
      ...
      // usando as relações recursivas: x(n)=p.x(n-1)-q.x(n-2),com U(0)=1,
      // U(1)=1 e V(0)=2, V(1)=P
      int64_t U2 = P * U1 - Q * U0, V2 = P * V1 - Q * V0;

      // Assegurando que os termos da sequência estão dentro do limite de
      // validade
      if (U1 % 2 == 0)
        U2 %= n;
      else
...
baillie_psw_test (uint64_t n)
{
  ...
  if (n == 2 || n == 3 || n == 5 || n == 7 || n == 13 || n == 17 || n == 19
      ...
      || n == 47)
    return true;
  if (n == 53 || n == 59 || n == 61 || n == 67 || n == 71 || n == 73 || n == 79
      || n == 83 || n == 89 || n == 97)
    return true;
