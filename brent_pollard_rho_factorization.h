// VAMOS CRIAR UM PROGRAMA QUE IMPLEMENTA A VARIANTE BRENT DO ALGORITMO DE
// POLLARD RHO PARA FATORAR NÚMEROS INTEIROS

/*
O ALGORITMO DE BRENT É UMA VARIANTE DO ALGORITMO DE POLLARD RHO PARA FATORAR
NÚMEROS INTEIROS. AO PASSO QUE NO ALGORITMO DE POLLARD RHO USA-SE DUAS
SEQUÊNCIAS DE INTEIROS PSEUDOALETÓRIAS x(i) E y(i), DE MODO QUE CICLOS SÃO
DETECTADOS VIA mdc(|x(i)-y(i)|, n), ONDE n É O NÚMERO A SER FATORADO, NO
ALGORITMO DE BRENT USA-SE DOIS PONTEIROS QUE SÃO LIMITADOS POR SEQUÊNCIAS DE
POTÊNCIAS DE 2. TAL MODIFICAÇÃO NA DETECÇÃO DE CICLOS, OTIMIZA O ALGORITMO
ORIGINAL DE POLLARD EM APROXIDAMENTE 25% E TAMBÉM É ÚTIL EM CERTAS CLASSES DE
ALGORITMOS DE GERAÇÃO DE NÚMEROS PSEUALEATÓRIOS.
...
void brent_pollard_rho_factorization(__int128_t number) {
  ...
  for (i = 1; i < UINT64_MAX; ++i) {
    ...
    if (power2_checker(i) == true) xm = xi;
  };
