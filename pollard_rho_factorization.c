...
O ALGORITMO BASEIA-SE NA GERAÇÃO GERAÇÃO DE UMA SEQUÊNCIA PSEUDO-ALEATÓRIA USANDO UM POLONÔMIO COMO p(x)=x²+1
OU p(x)=x²+1 mod(n), ONDE n É UM NÚMERO A SER FATORADO.
...
uint64_t euclides_check(uint64_t a, uint64_t b){
  ...
  if(b==0) return a;
  else return euclides_check(b, a%b);
                                               };
...
void pollard_rho(uint64_t x, uint64_t y, uint64_t n){
  ...
  //Procedimentos
    ...
    while(remainder==1){
      ...
      if(x>y) remainder=euclides_check((x-y), n);
      if(x<y) remainder=euclides_check((y-x), n);
