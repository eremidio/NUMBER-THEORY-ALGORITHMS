...
/*
O ALGORITMO DE POLLARD RHO É UM EFICIENTE E PROBABILÍSTICO ALGORITMO PARA O CÁLCULO DE LOGARITMOS DISCRETOS SIMILAR AO ALGORITMO DE MESMO NOME USADO
PARA FATORAR NÚMEROS INTEIROS.
 
O ALGORITMO RESOLVE O SEGUINTE PROBLEMA SEJA G UM GRUPO DE ORDEM p-1, POR EXEMPLO, INTEIROS A MENOS DE UMA CONGRUÊNCIA MÓDULO p. SEJAM g UM GERADOR
DESTE GRUPO E h UM ELEMENTO DESTE GRUPO ENTÃO EXISTE UM x TAL QUE g^x=h. PARA COMPUTAR x O ALGORITMO USA UMA FUNÇÃO QUE GERA DUAS TRIPLAS DE NÚMEROS
PSEUDOALEÁTÓRIOS (x1, a1, b1) E (x2, a2, b2) EM UM CONJUNTO FINITO S={0, ..., (p-1)}. SE f FOR UMA FUNÇÃO GERADORA AS SEQUÊNCIAS SÃO ATUALIZADAS
USANDO-SE AS REGRAS: (x1, a1, b1) -> f(x1, a1, b1) E (x2, a2, b2) -> f(x2, a2, b2) -> f²(x2, a2, b2). SE APÓS  UM CERTO NÚMERO DE ITERAÇÕES FOR POSSÍVEL
ENCONTRAR VALORES x1=x2 (AS DUAS SEQUÊNCIAS COLIDEM), ENTÃO UMA IDENTIDADE DO TIPO (b1-b2) x =(a1-a2) (mod p) É SATISFEITA.
TAL EQUAÇÃO É FACILMENTE RESOLVIDA PELO ALGORITMO DE EUCLIDES EXTENDIDO.
...
void pseudo_randomwalk(int64_t* x, int64_t* a, int64_t* b, int64_t n, int64_t g, int64_t h){
 ...
 //Procedimentos: Atualização de variáveis
