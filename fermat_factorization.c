...
//COMPILAR ESTE PROGRAMA COM O COMANDO: gcc -o fermat_factorization fermat_factorization.c -lm
/*
ESTE ALGORITMO USA O FATODE QUE PARA NÚMEROS INTEIROS ÍMPARES PODEM SER ESCRITOS COMO A DIFERENÇA DE DOIS QUADRADOS.
SEJA n UM INTEIRO COMPOSTO ÍMPAR, TEMOS QUE n=a²-b², ONDE a>n^(1/2) E b<n^(1/2).
PARA NÚMEROS PARES n UMA DECOMPOSIÇÃO TRIVIAL É n/2, 2.
*/
...
uint64_t limit(uint64_t number){
return floor(sqrt(number));
                               };

//Função que executa o algoritmo de Fermat
void fermat_algorithm(uint64_t number){
//Variáveis locais
...
if(i>pow(number, 4)){
printf("O algoritmo não encontrou fatores do número em questão dentro do limite definido.\n");
break;
                    };
