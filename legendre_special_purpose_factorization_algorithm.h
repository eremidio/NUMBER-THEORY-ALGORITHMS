...
/*
O SEGUINTE TEOREMA DEVIDO A LEGENDRE É ÚTIL PARA FAORAR NÚMEROS INTEIROS DAS
FORMAS n=(a^k)+(b^k) OU n=(a^k)-(b^k), COM a,b,k INTEIROS E mdc(a,b)=1. TEOREMA:
SEJA n=(a^k)+(b^k) OU n=(a^k)-(b^k), COM a,b,k INTEIROS E mdc(a,b)=1, A MENOS DE
FATORES ALGÉBRICOS, TODOS OS FATORES PRIMOS DE n TEM A FORMA p=lk+1 PARA l
INTEIRO. SE DECONSIDERAMOS OS CASOS a,b=2 E  n=(a^k)-(b^k) COM k PAR TÊM-SE QUE
p=2lk+1.

TAL TEOREMA É PARTICULARMENTE ÚTIL NA BUSCA DE FATORES PRIMOS DE NÚMEROS DE
FORMA ESPECIAL DADA ACIMA. USAREMOS O TESTE AQUI PARA INTEIROS DE 64 BITS.
...
void legendre_special_purpose_factorization_algorithm(uint64_t, uint64_t,
                                                      uint64_t, signed);
...
// Função
void legendre_special_purpose_factorization_algorithm(uint64_t a, uint64_t b,
                                                      uint64_t k,
                                                      signed signal) {
  // Variáveis locais
  uint64_t N, root;
