...
NÃO É VIÁVEL, POIS O NÚMERO DE TERMOS CRESCE EXPONENCIALMENTE COMO VALOR DE n. ESCOLHENDO UM VALOR DE r ADEQUADO ESCREVEMOS (x+a)^n = f(x)q(x)+s(x), ONDE f(x), q(x), s(x) SÃO POLINÔMIOS E r(x) TEM GRAU (r-1). O TESTE SE REDUZ ENTÃO A TESTES SOBRE OS COEFICIENTES DE s(x). ALGUNS TESTES ADICIONAIS SÃO PERFORMADOS  ENVOLVENDO A ORDEM
MULTIPLICATIVA DE n PARA DETECTAR A PRIMALIDADE DO NÚMERO EM QUESTÃO.
