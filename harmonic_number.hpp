...
/*
O n-ÉSIMO NÚMERO HARMÔNICO É DEFINIDO PELA RELAÇÃO: H(n)=1+(1/2)+(1/3)+ ...
+(1/n). AS SEGUINTES RELAÇÕES DE RECORRÊNCIA SÃO UTÉIS EM COMPUTAR OS NÚMEROS
HARMÔNICOS: H(n+1)=H(n)+1/(n+1); H(n-1)=H(n)-(1/n). DONDE
H(n+k)=H(n)+1/(n+1)+1/(n+2)+ ... +1/(n+k)

PARA n TENDENDO A INFINITO TEMOS: H(n) ~ H(n+1) ~ H(n+2) ~ ... ~ H(n+k), COM k
INTEIRO. ADMITINDO QUE TAL RELAÇÃO SEJA VÁLIDA PARA NÚMEROS REAIS. PODE-SE
EXTENDER A DEFINIÇÃO DE NÚMEROS HARMÔNICOS PARA NÚMEROS REAIS E COMPLEXOS. DE
FATO PARA n SUFICIENTEMENTE GRANDE E x REAL TÊM-SE: H(n+x)-H(x)~0 -->
H(x)~(H(n)-(H(n+x)-H(x))) USANDO AS RELAÇÕES DE RECORRÊNCIA ACIMA OBTÉM-SE:
H(x)=Σ (1/k)-(1/(x+k))=x.Σ(1/(k(x+k))

O MESMO RACIOCÍNIO SE PALICA ANÚMEROS COMPLEXOS.

A INTEGRAL DEFINIDA DE 0 A 1 DE H(x) NOS DÁ A CONSTANTE DE EULER-MASCHERONI,
CUJO VALOR É γ≃ 0.57721566490153286060651209008240243104215933593992. ESTA
CONSTANTE PODE SER DEFINIDA COMO: γ = H(n) -ln(n) COM n TENDENDO A INFINITO.

OS NÚMEROS HARMÔNICOS POSSUEM RELAÇÃO DIRETA COM FUNÇÕES IMPORTANTES EM
MATEMÁTICA COMO A FUNÇÃO DIGAMA (DERIVADA DO LOGARITMO DA FUNÇÃO GAMA) E A
FUNÇÃO ZETA DE RIEMANN (DE FATO, NÚMEROS HARMÔNICOS SÃO SOMAS PARCIAIS DA SÉRIE
HARMÔNICA QUE É NUMERICAMENTE IGUAL A ζ(1)).
...
complex<double>
harmonic_number (complex<double> z)
{
...
digamma (double x)
{
  ...
  double parcel7
      = (691.0 / (32760.0 * x * x * x * x * x * x * x * x * x * x * x * x));
  double parcel8
      = (1.0 / (12.0 * x * x * x * x * x * x * x * x * x * x * x * x * x * x));
  return (std::log (x) - parcel1 - parcel2 + parcel3 - parcel4 + parcel5
          - parcel6 + parcel7 - parcel8);
};

// Argumentos complexos

complex<double>
digamma (complex<double> z)
{
  // Constante
  ...
  // Caso base
  if (z.real < 6.0)
    return digamma (z + hum) - z.inv ();

  // Caso geral
  complex<double> log_z (std::log (z.norm ()), z.polar ());
  complex<double> coefficient;
  complex<double> parcel1, parcel2, parcel3, parcel4, parcel5, parcel6,
      parcel7, parcel8;
...
double
euler_mascheroni_approximation (uint64_t n)
{
