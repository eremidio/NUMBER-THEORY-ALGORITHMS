...
/*
USANDO A TEORIA DAS FORMAS QUADRÁTICAS BINÁRIAS É POSSÍVEL DEMONSTRAR UMA SÉRIE DE TEOREMAS QUE ESTABELECEM RELAÇÕES DE CONGRUÊNCIAS DO TIPO p= a (mod m), COM p PRIMO E
a, m INTEIROS E NÃO NECESSARIAMENTE PRIMOS QUE DEFINEM CONDIÇÕES PARA QUE p SEJA EXPRESSO POR MEIO DE RELAÇÕES DO TIPO p=x²+ny², COM x, y, n INTEIROS PARA VALORES FIXOS
DE n. OS VALORES DE x, y PODEM SER OBTIDOS POR MEIO DO ALGORITMO DE CONNARCHIA.
...
#include"connarcchia_algorithm.h"//Dependências necessárias estão inclusas
#include<stdio.h>
...
//Resultado
if(residue==1  || residue==9 || residue==17 || residue==19  || residue==25 || residue==29 || residue==49)
return true;
else 
return false;
                      };

//Função que testa se um primo pode ser escrito na forma p=x²+14y² (ou 2p=x²+14y²)
bool test14(int64_t p){
//Variáveis locais
int64_t residue=(p%56);

//Resultado
if(residue==1  || residue==9 || residue==15 || residue==23 || residue==25 || residue==39)
return true;
else 
return false;
                      };
...
//Resultado
if(residue==1 || residue==9 || residue==15 || residue==23 || residue==25 || residue==31 || residue==47 || residue==49 || residue==71 || residue==81)
return true;
else 
return false;
                      };
...
//Função que expressa números em termos deformas quadráticas reduzidas 
