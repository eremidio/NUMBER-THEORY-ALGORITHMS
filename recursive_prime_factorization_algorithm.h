...
A IDEIA PRINCIPAL DO ALGORITMO É USAR A RELAÇÃO RECURSIVA: a(n)=a(n-1)+mdc(n, a(n-1)) QUE PRODUZ UMA SEQUÊNCIA DE NÚMEROS PRIMOS OU FATORES 1's.
...
PROPOSIÇÃO: SEJA N, UM NUMÉRO NATURAL ÍMPAR QUALQUER TAL QUE N>2 E SEJA A SEQUÊNCIA a(n)=a(n-1)+mdc(n, a(n-1)). SE PARA n<N^(1/2)+1, mdc(n, a(n-1))>2. ENTÃO mdc(n, a(n-1)) É UM FATOR NÃO 
TRIVIAL DE N.
...
O(N^(1/2)ln(N)) QUANDO N FOR PRIMO. NÃO É, PORTANTO, ADEQUADO A FATORAÇÃO DE NÚMEROS INTEIROS GRANDES, PORÉM NAS MÁQUINAS MODERNAS É RAZOÁVEL PARA NÚMEROS DA ORDEM DE 10^16 ~
10^18.
