...
//COMPILAR ESTE PROGRAMA COM O COMANDO: gcc -o lucas_pseudoprime_test lucas_pseudoprime_test.c -lm
...
/*
 ...
 6. COMPUTA-SE O mdc DO k-ÉSIMO TERMO E DE n, CASO O RESULTADO SEJA 1 O NÚMERO EM QUESTÃO É UM PROVÁVEL PRIMO.
...
//Função que calcula o k-ésimo termo na sequência de Lucas a menos de uma relação de congruência
uint64_t lucas_sequence(uint64_t n, uint64_t P, uint64_t Q, uint64_t k){
 ...
 //Procedimentos
  //Computando as sequências de Lucas a menos de uma congruência 
  for(i=0; i<k; ++i){
   U2=(((P*U1)%n)-((Q*U0)%n))%n;
