...
/*
AS CHAMADAS SEQUÊNCIAS DE FAREY SÃO SEQUÊNCIAS DE FRAÇÕES COMPLETAMENTE
REDUZIDAS. ESTAS SEQUÊNCIAS NORMALMENTE SÃO RESTRITAS ENTRE 0 E 1.

AS SEQUÊNCIAS DE FAREY SÃO MUITO IMPORTANTES EM DIVERSAS ÁREAS DA MATEMÁTICA
TENDO UMA RELAÇÃO DIRETA COM A CONJECTURA DE COLLATZ, A HIPÓTESE DE RIEMANN,
APROXIMAÇÃO DE IRRACIONAIS, CICLOS DE FORD EM GEOMETRIA, TEORIA DOS FRACTAIS,
ALÉM DE APARECER EM UMA SÉRIE DE RELAÇÕES EXTREMAMENTE ÚTEIS ENVOLVENDO FUNÇÕES
ESPECIAIS.
...
//*************************************************************************************************************************
// FUNÇÕES
// Função que implementa o algoritmo de Euclides para cálculo do mmc  de dois
// números
int
euclides_algorithm (int a, int b)
{
  if (b == 0)
    return a;
  else
...
bool
compare_fractions (fraction a, fraction b)
{
...
void
generate_farey_sequence (int n, enum ordering x)
{
  // Variáveis locais
  fraction element;
  ...
  if (x == Up)
    std::sort (fraction_vector.begin (), fraction_vector.end (),
               compare_fractions);
  if (x == Down)
    {
      std::sort (fraction_vector.begin (), fraction_vector.end (),
