...
/*
USANDO A LEI DA RECIPROCIDADE QUADRÁTICA DE GAUSS E O CRITÉRIO DE EULER UM
SIMPLES TESTE DE PRIMALIDADE É DEFINIDO PARA INTEIROS DE 64 BITS. SEJA (a|q) O
SÍMBOLO DE LEGENDRE, A LEI DE RECIPROCIDADE QUADRÁTICA AFIRMA QUE PARA DOIS
PRIMOS ÍMPARES p E q (p|q)(q|p)={(-1)^((p-1)/2)}{(-1)^((q-1)/2)}.

O CRITÉRIO DE EULER ESTABELECE QUE PARA UM INTEIRO a COPRIMO COM UM PRIMO ÍMPAR
p: (a|p)=a^((p-1)/2) (mod p).

O SEGUINTE TEOREMA É USADO NO TESTE DE  PRIMALIDADE EM QUESTÃO:
PARA UM PRIMO ÍMPAR p: (2|p)=+1 SE p=7 (mod 8) OU p=1 (mod 8); (2|p)=(-1) SE p=3
(mod 8) OU p=5 (mod 8).

OS RESULTADOS ACIMA NOS PERMITE TESTAR A PRIMALIDADE DE UM INTEIRO DE 64 BITS
USANDO SIMPLES OPERAÇÕES DE EXPONENCIAÇÃO E ARITMÉTICA MODULAR. SE EQUIPADO COM
O TESTE DE MILLER-RABIN É POSSSÍVEL CRIAR UM EFICIENTE MÉTODO DETERMINÍSTICO
PARA TESTAR A PRIMALIDADE DE INTEIROS DE ATÉ 64 BITS.
...
bool gauss_euler_primality_test(uint64_t n) {
  ...
  // Teste 1
  if ((n % 8) == 1 || (n % 8) == 7) {
    if (mod_bin_pow(2, ((n - 1) / 2), n) != 1) return false;
  };

  // Teste 2
  if ((n % 8) == 3 || (n % 8) == 5) {
    if (mod_bin_pow(2, ((n - 1) / 2), n) != (n - 1)) return false;
  };
  ...
  // Teste 4
  if (tester2 != 1 && tester2 != (n - 1)) return false;
  ...
  // Teste 9
  if (mod_bin_pow(prime, ((n - 1) / 2), n) != (n - 1)) return false;
