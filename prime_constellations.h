...
ÍMPAR, ENTÃO NECESSARIAMENTE (p+1) É PAR E PORTANTO DIVISÍVEL POR 2. UM RACIOCÍNIO SIMILAR PERMITE INFERIR QUE AS TRIPLAS {p, p+2; p+4} NÃO
...
VALORES PODEM SER APROXIMADAMENTE COMPUTADOS USANDO-SE AS CONJECTURAS DE HARDY-LITTLEWOOD. BASEADOS NA IDEIA DE QUE A PROBABILIDADE DE 
NÚMEROS INTEIROS SEREM PRIMOS EM TORNO DE UM VALOR x É DE 1/ln(x), O NÚMERO DE OCORRÊNCIAS UM DETERMINADA CONSTELAÇÃO CONTENDO N-UPLAS COM
'N' ELEMENTOS É DEFINIDA POR MEIO DE EXPRESSÕES DO TIPO Cp.I(x), ONDE I(x) É A INTEGRAL DEFINIDA DE 2 A x DA FUNÇÃO f(x)=ln(x)^{-N} E Cp SÃO
AS CHAMADAS CONSTANTES DE HARDY-LITTLEWOOD. AS CONSTANTES Cp PARA {a(1), ..., a(n)} PODEM SER COMPUTADAS USANDO-SE A FÓRMULA
Cp = {2^n}.Π [1-w(p; a(1), ..., a(n))]/p}/[(1-1/p)^{n+1}], ONDE O PRODUTÓRIO É FEITO SOBRE TODOS OS PRIMOS p E w(p; a(1), ..., a(n)) É O
VALOR DO PRODUTO DO NÚMERO DE CLASSES RESIDUAIS a(i) mod p (IGUAL A p-n PARA PRIMOS SUFICIENTEMENTE GRANDES). A INTEGRAL INDEFINIDA DE
f(x)=ln(x)^{-N} É DADA A MENOS DE UMA CONSTANTE PELA FÓRMULA POR I(x) = li(x) - x.Σ[k!/ln(x)^{k+1}] COM k=1, .., (N-2); ONDE li(x) É A
